...
uint8_t can_send_pixel1_16bit( uint8_t update_mode, uint8_t panel_address, uint32_t *rgb, uint8_t* pixel_address){
	uint8_t nbytes;
	struct can_frame frame;

	frame.can_id  = ((panel_address) << 3) + update_mode + (pixel_address[0] << 25) + 0x80000000;
	frame.can_dlc = 2;
	
		uint16_t p = to565(rgb[0]);
		frame.data[0] = p >> 8;
		frame.data[1] = p & 0xFF; 
...
uint8_t can_send_pixel2_16bit( uint8_t update_mode, uint8_t panel_address, uint32_t *rgb, uint8_t* pixel_address){
	uint8_t nbytes;
	struct can_frame frame;
...
uint8_t can_send_pixel3_16bit( uint8_t update_mode, uint8_t panel_address, uint32_t *rgb, uint8_t* pixel_address){
	uint8_t nbytes;
	struct can_frame frame;
